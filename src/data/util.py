...
def get_skel(df):
    if len(list(itertools.chain(*df.limbs_subset)))>0:
        ...
        peak_df = pd.DataFrame.from_dict({'x_pred':x,'y_pred':y,'c':c,'peak_idx':peak_idx,'part_idx':kp_idx})
...
def get_prediction(keras_weights_file, model_name, save_images, image_path):
    image_save_path = os.path.join('../../data/interim/images', model_name)
...
#     image_list = image_list[:2]
    ...
    for input_image in image_list:
        ...
        try:
            ...
            output_dict.update({'file_path':input_image, 'video':video_number, 'frame':frame_number, 'id':int(image_id)})
    ...
    counts = kp_pred_df.groupby(['id', 'person_idx'])['c'].count().reset_index()
    max_rows = counts.groupby('id')['c'].idxmax().tolist()
    max_rows_df = counts.loc[max_rows,['id', 'person_idx']]
    max_rows_df['dum'] = 1
    kp_pred_df = pd.merge(kp_pred_df, max_rows_df, on=['id', 'person_idx'], how='inner')
    ...
    columns = pd.MultiIndex.from_product([[model_name],['x','y']], names=['var_type','dim'])
    index = pd.MultiIndex.from_arrays([np.array(kp_pred_df.id), np.array(kp_pred_df.part_label)], names=['id','part_label'])
    kp_pred_df = pd.DataFrame(kp_pred_df[['x_pred', 'y_pred']].as_matrix(),index=index, columns=columns)
    return kp_pred_df

def get_image_meta_data_from_filenames(path):
    files = glob.glob(os.path.join(path, '*'))
    file_series = pd.Series([os.path.basename(i).split('.')[0] for i in files])
    df = pd.DataFrame(pd.Series(files), columns = ['image_path'])
    df['frame'] = file_series.str[7:].astype(int)
    df['video'] = file_series.str[1:7].astype(int)
    return df
...
def get_ground_truth(path):
    ...
    df = pd.DataFrame.from_records(data['annotations'])
    return df

def get_keypoints(df):
    part_gt_labels = pd.Series(['nose','left_eye','right_eye','left_ear','right_ear','left_shoulder','right_shoulder',\
'left_elbow','right_elbow','left_wrist','right_wrist','left_hip','right_hip','left_knee',\
'right_knee','left_ankle','right_ankle'])
    df = df.reset_index(drop=True)
    x = pd.Series(df.loc[0,'keypoints'][0::3])
    y = pd.Series(df.loc[0,'keypoints'][1::3])
    zdf = pd.DataFrame.from_dict({'part_label':part_gt_labels, 'x_gt':x, 'y_gt':y})
    return zdf
